...
class Passenger:
    ...
    def __str__(self):
        return f"Passenger's information: Name: {self.name}, Age: {self.age}, Passport ID: {self.passport_id} "
...
class Flight:
    def __init__(self, flight_number, departure, destination, departure_time, aircraft_type):
        ...
        elif self.aircraft_type == "A350":      # seat map for A350
            self.business_seats = 29
            self.premium_economy_seats = 45
            self.economy_seats = 231
        elif self.aircraft_type == "B787":      # seat map for B787
    ...
    def book_seat(self, seat_class):       # when book seat, the seat in correlated class -= 1 and reserved seat += 1
        if seat_class == "Business" and self.business_seats > 0:
            ...
            print("Class chosen out of seat, please choose another option")
            return False
...
class Reservation(Passenger):
    ...
    def __str__(self):
        return (f"Reservation(Passenger: {self.name}, Flight: {self.flight.flight}, "
                f"Class: {self.seat})")
